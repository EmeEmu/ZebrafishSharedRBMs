...
# ╔═╡ 505c50ef-f977-4c96-b5bf-d389cffee346
md"""
## 2.3. Training with dropout fish
"""

# ╔═╡ 3640f899-1da1-4727-b292-69780e63229b
md"Lauch multiple trainings ? $(@bind lauch_multi CheckBox(default=false))"

# ╔═╡ 0b396f88-35a3-469b-a9a7-7a933332be55
if lauch_multi
	for fish in FISH
		dropedFISH = filter(x->x≠fish, FISH)
		spikes = hcat(
			[load_data(
				LOAD.load_dataVOX(fish, voxel_size)
			).spikes for fish in dropedFISH]...
		)
		dataset = Data(
			"multivoxelized_$(length(FISH))fish_droped$(fish)_$(voxel_size)vox",
			spikes,
...
# ╔═╡ 37c3a862-be96-4d19-8729-eccc691fb732
